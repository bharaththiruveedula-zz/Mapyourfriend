...
<?php
	function uploadImage($fileName, $maxSize, $maxW, $fullPath, $relPath, $colorR, $colorG, $colorB, $maxH = null){
		$folder = $relPath.'/'.$_SESSION['groupid'].'/';
		$maxlimit = $maxSize;
		...
		if($filesize > 0){	
			...
			if ( file_exists("../../res/usr_img/".$_SESSION['groupid'].'/'.$_SESSION['name'].'_'.$_SESSION['groupid'].'.jpg') ) {
			unlink("../../res/usr_img/".$_SESSION['groupid'].'/'.$_SESSION['name'].'_'.$_SESSION['groupid'].'.jpg');
			}
			...
			if(count($errorList)<1){
				...
				foreach($allowed_ext as $ext){
					if($ext==end($file_ext)){
						...
						$newfilename = $_SESSION['name'].'_'.$_SESSION['groupid'].'.jpg';
...
<?php
	if($imgUploaded){
		echo '<img src="../../res/img/success.gif" width="16" height="16" border="0" style="marin-bottom: -4px;" /> Success!<br /><img src="'.$upload_image.'" border="0" />';
	    $query = "INSERT INTO IMAGE VALUES ('".$_SESSION[name]."_".$_SESSION[groupid]."')";
	     $result=mysql_query($query);

	}else{
		echo '<img src="../../res/img/error.gif" width="16" height="16px" border="0" style="marin-bottom: -3px;" /> Error(s) Found: ';
